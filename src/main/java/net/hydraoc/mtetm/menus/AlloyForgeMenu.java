...
public class AlloyForgeMenu extends AbstractContainerMenu {
    ...
    public AlloyForgeMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public AlloyForgeMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.HELLFORGE_MENU.get(), pContainerId);
        checkContainerSize(inv, 6);
        blockEntity = ((AlloyForgeBlockEntity) entity);
